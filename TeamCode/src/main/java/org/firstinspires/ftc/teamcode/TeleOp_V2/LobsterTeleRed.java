...
@TeleOp(name = "LobsterTeleRed")
public class LobsterTeleRed extends NextFTCOpMode{

    private LaserSubsystem laser;
    Gamepad.RumbleEffect customRumbleEffect;    // Use to build a custom rumble sequence.

    public LobsterTeleRed() {
        addComponents(
...
//                .whenBecomesFalse(SubServoTurret.INSTANCE.middle);
    ...
    public void onUpdate(){
        ...
        if (gamepad1.dpadUpWasPressed()){
            PedroComponent.follower().setPose(startingPoseRed);
        }
        ...
        telemetry.addData("dist from goal", PedroComponent.follower().getPose().distanceFrom(RedGOAL));
